...
const UpdateStatus = () => {
   ...
   const handleValidationStatus = async (isValid, uniqueId = '') => {
      ...
      try {
         ...
         const { data } = await axios.get(`/api/v1/status/find/${uniqueId}`);
         ...
         if (data && data.success) {
            ...
            throw new Error(data?.message || 'Student status not found');
         }
      } catch (error) {
         console.error('Error fetching student status:', error);
         toast.error('Failed to fetch student status: ' + (error.response?.data?.message || error.message));
         setCurrentStatus('');
         setSelectedStatus('');
         setReviewMessage('');
      } finally {
   ...
   const handleSubmit = async (e) => {
      ...
      if (!isValidStudentId || !validatedUniqueId) {
         toast.error('Please enter a valid Student ID before submitting.');
         return;
      }
      ...
      if (selectedStatus === currentStatus) {
         toast.error('Please select a different status. Current status is already "' + currentStatus + '".');
         return;
      }

      if (!reviewMessage.trim()) {
         toast.error('Please provide a review message for the student.');
         return;
      }
      ...
      try {
         ...
         if (response.data && response.data.success) {
            ...
            toast.success(`Student application status successfully updated to "${selectedStatus}"!`);
         } else {
            throw new Error(response.data?.message || 'Failed to update status');
         }
      } catch (error) {
         console.error('Error updating status:', error);
         toast.error('Failed to update student status: ' + (error.response?.data?.message || error.message));
      } finally {
         setLoading(false);
      }
   };
            ...
            <div className='w-[94%] max-[553px]:w-[87%] mb-6'>
               <FormTemplate_Secondary>
                  <div className='bg-[#090909] rounded-tr-xl rounded-tl-xl p-4 w-full text-center'>
                     <h2 className="text-xl font-semibold text-white">Update Student Application Status</h2>
                  </div>
                  
                  <form onSubmit={handleSubmit} className='w-full flex flex-col items-center justify-center mt-4 px-[2vw] pb-8'>
                        ...
                        <div className='w-full flex item-center gap-6 mb-6'>
                           ...
                           <div className="">
                              <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${
                                 currentStatus === 'Pending' ? 'bg-yellow-900/30 text-yellow-300' :
                                 currentStatus === 'Verified' ? 'bg-blue-900/30 text-blue-300' :
                                 currentStatus === 'Approved' ? 'bg-green-900/30 text-green-300' :
                                 currentStatus === 'Rejected' ? 'bg-red-900/30 text-red-300' :
                     ...
                     <div className="w-full flex justify-center mb-4">
                        <button
                           type="submit"
                           disabled={!isValidStudentId || !selectedStatus || !reviewMessage.trim() || loading || selectedStatus === currentStatus}
                           className={`
                              py-3 px-6 rounded-md font-semibold text-lg transition-all duration-200
                              ${isValidStudentId && selectedStatus && reviewMessage.trim() && !loading && selectedStatus !== currentStatus
                     ...
                     {!isValidStudentId && (
                        <div className="w-[95%] text-sm p-4 rounded-md border bg-yellow-900/30 text-yellow-300 border-yellow-600">
                           Please enter a valid Student Unique ID to enable status update and review message fields.
                        </div>
                     )}
                  </form>
               </FormTemplate_Secondary>
